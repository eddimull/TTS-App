...
// with .value — returns null while loading or on error, which is fine
// since we treat null bandId as "not ready" and return an empty list.

final songsProvider = FutureProvider<List<SongResult>>((ref) async {
...
final chartsProvider =
    FutureProvider<List<ChartResult>>((ref) async {
  final bandId = ref.watch(selectedBandProvider).value;
  if (bandId == null) return [];
